MODULE CryptoBigNumbers;
        ...
        BEGIN
                FOR i := 0 TO n - 1 DO
                        buf[p + i] := CHR(ENTIER(randomgenerator.Uniform() * 256))
                END
        END RandomBytes;
        ...
        BEGIN
                IF a[1] = 0 THEN
                        IF (a[0] < 80000000H) & (b < 80000000H) THEN
                                RETURN INTEGER(a[0] DIV b)
                        ELSIF a[0] < b THEN
                                RETURN 0
                        ELSIF a[0] = b THEN
                                RETURN 1
                ...
                q := {};
                r := 0;
                WHILE (bit >= 0) & ~(bit MOD 32 IN SET32(a[bit DIV 32])) DO
                        DEC(bit)
                END;
                WHILE bit >= 0 DO
                        overflow := 31 IN SET32(r);
                        r := ASH(r, 1);
                        IF bit MOD 32 IN SET32(a[bit DIV 32]) THEN
                                INC(r)
                        END;
                        IF overflow OR (b <= r) THEN
                                r := r - b;
                                IF bit < 32 THEN
                                        INCL(q, bit)
                                ELSE
        ...
        PROCEDURE div96(CONST a : dig3; CONST b : dig2): INTEGER;
                ...
                BEGIN
                        overflow := 31 IN SET32(x[1]);
                        x[1] := ASH(x[1], 1);
                        IF 31 IN SET32(x[0]) THEN
                                INC(x[1])
                        END;
                        x[0] := ASH(x[0], 1)
                END shift;
